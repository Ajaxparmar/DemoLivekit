...
export const PlaygroundTile: React.FC<PlaygroundTileProps> = ({
  ...
  return (
    <div
      className={`flex flex-col  rounded-sm border-gray-800 text-gray-500 bg-${backgroundColor} ${className}`}
    >
      {title && (
        <div
          className="all-title flex items-center justify-center text-xs uppercase py-2 border-b border-b-gray-800 tracking-wider"
          style={{
...
export const PlaygroundTabbedTile: React.FC<PlaygroundTabbedTileProps> = ({
  ...
  return (
    <div
      className={`flex flex-col h-full  rounded-sm border-gray-800 text-gray-500 bg-${backgroundColor} ${className}`}
    >
      <div
        className="flex items-center justify-start text-xs uppercase border-b border-b-gray-800 tracking-wider"
        style={{
          height: `${titleHeight}px`, display:'none',maxHeight:'200px',
        }}
      >
        {tabs.map((tab, index) => (
          <button
